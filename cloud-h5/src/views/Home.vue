<template>
  <div class="container_box" id="video_box">
    <a href>
      <img
        src="http://npjy.oss-cn-beijing.aliyuncs.com/images/file-15807175142891GIFp.png"
        class="back_i"
        alt
      />
    </a>
      ...
      .then(() => {
        ...
        cloud
          ...
          .then(res => {
            ...
            let videoItem = res.result.data;
            res.result.data.forEach(elem => {
              elem.des = elem.title;
              elem.author = elem.auth;
              (elem.cover =
                "http://oss.jishiyoo.com/images/file-1575341210559.png"), //封面
                (elem.tag_image =
                  "http://npjy.oss-cn-beijing.aliyuncs.com/images/file-1575449277018pF3XL.jpg"), //作者头像
                cloud
                  .getTempFileURL({
                    fileList: [elem.videoUrl]
                  })
                  .then(res => {
                
                    elem.url = res.fileList[0].tempFileURL;
                        console.log("获取连接==》", elem);
                        that.$data.videoList.push(elem)
                  });
            });
          });
      })
